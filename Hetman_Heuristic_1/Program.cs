...
//ZACZYNAMY W PRAWYM górnym ROGU
namespace Hetman_Heuristic_1
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("TESTY DLA HETMANOW - Heurystyka 1");
    ...
    class H_Solver
    {
        ...
        public bool isHetmanPlacementOk(int row_, int col_)
        {
            ...
            for (int c = col_ + 1, r = row_ - 1; c < N && r > -1; c++, r--)
            {
                if (columns[c].row == r) return false;
            }

            for (int c = col_ + 1, r = row_ + 1; c < N && r < N; c++, r++)
            {
                if (columns[c].row == r) return false;
            }
        ...
        public bool isDwo(H_Spot hs, List<List<H_Spot>> dom_changes, int row)
        {
            ...
            for (int c = col - 1, r = row - 1; c > -1 && r > -1; c--, r--)
            {
                if (columns[c].isMyDwo()) return true;
            ...
            for (int c = col - 1, r = row + 1; c > -1 && r < N; c--, r++)
            {
                if (columns[c].isMyDwo()) return true;
                if (columns[c].my_domain.Contains(r))
                {
        ...
        private void solve_ForwardChecking(int col)
        {
            if (col != -1)
            {
                ...
                for (int i = 0; i < hs.my_domain.Count; i++)
                {
                    ...
                    List<List<H_Spot>> dom_changes = createDomainChangesSave();
                    bool dwo = isDwo(hs, dom_changes, r);
                        if (!dwo)
                        {
                            columns[col].row = r;
                    ...
                    foreach (int row in board_domain)
                    {
                        foreach (H_Spot h in dom_changes[row])
                        {
