...
//SPIRALA OD ZEWNĄTRZ + liczby parzyste najpierw
namespace Latin_Square_Heuristic_2
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("TESTY DLA LAITN SQUARE - heurystyka 2");
    ...
    class LS_Solver
    {
        ...
        public bool isDwo(Variable variable, List<Variable> dom_changes, int num)
        {
            ...
            for (int r = 0; r < N; r++)
            {
                if (r == row) r++;
                if (r >= N) break;
                if (board[r, col].number == Variable.EMPTY && board[r, col].my_domain.Contains(num) && r != row)
                {
            ...
            for (int c = 0; c < N; c++)
            {
                if (c == col) c++;
                if (c >= N) break;
                if (board[row, c].number == Variable.EMPTY && board[row, c].my_domain.Contains(num))
                {
